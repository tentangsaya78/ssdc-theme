...
<div x-data="{openContact: false}">
    ...
    <section class="py-16 bg-primary" x-data="{ modalOpen: false, embedUrl: '', fileUrl: '', isExternal: false }">
                        ...
                        foreach ($videos as $video_item):
                            ...
                            <li class="splide__slide">
                                <button class="relative flex justify-center items-center focus:outline-none group w-full"
                                    ...
                                    <img class="shadow-2xl w-full h-[460px] object-cover"
                                        src="<?php echo esc_url($video_thumbnail['url']); ?>" width="768" height="432"
                                        alt="<?php echo esc_attr($video_thumbnail['alt']); ?>" />
                                    <svg class="absolute pointer-events-none" xmlns="http://www.w3.org/2000/svg" width="72"
                                        height="72">
                                        <circle class="fill-primary/40" cx="36" cy="36" r="36" fill-opacity=".8" />
                                        <path class="fill-secondary"
                                            d="M44 36a.999.999 0 0 0-.427-.82l-10-7A1 1 0 0 0 32 29V43a.999.999 0 0 0 1.573.82l10-7A.995.995 0 0 0 44 36Z" />
                                    </svg>
                                </button>
                            </li>
                        <?php endforeach;
        ...
        <div style="display:none" x-show="modalOpen" @click="modalOpen = false; embedUrl = ''; fileUrl = '';"
            class="fixed inset-0 z-[99998] bg-black bg-opacity-50" aria-hidden="true">
        </div>

        <!-- Modal -->
        <div style="display:none" x-show="modalOpen"
            ...
            <div class="w-full max-w-5xl aspect-video bg-black shadow-2xl overflow-hidden pointer-events-auto">
                ...
                <template x-if="!isExternal">
                    <video x-effect="modalOpen ? ($el.load(), $el.play()) : ($el.pause(), $el.currentTime = 0)"
                        class="w-full h-full" loop controls autoplay>
